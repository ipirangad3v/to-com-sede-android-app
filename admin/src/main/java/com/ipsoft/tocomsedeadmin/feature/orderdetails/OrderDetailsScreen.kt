...
@Composable
fun AddressDetailsContainer(address: Address, status: OrderStatus) {
    Surface(color = Color.White) {
        Column(
            modifier = Modifier
                .fillMaxWidth()
            ...
            Text(
                text = if (status == OrderStatus.CONCLUDED) {
                    stringResource(id = R.string.delivered_at)
                } else {
                    stringResource(
                        id = R.string.delivery_at
                    )
                },
                style = MaterialTheme.typography.titleMedium
            )
    ...
    Surface(color = Color.White) {
        Column(
            ...
            if (order.paymentMethod == PaymentMethod.MONEY) {
                ...
                Row(modifier = Modifier.fillMaxWidth()) {
                    Text(
                        text = stringResource(id = R.string.change_for_client),
                        style = MaterialTheme.typography.bodyMedium,
                        modifier = Modifier.weight(1f)
                    )
                    Text(
